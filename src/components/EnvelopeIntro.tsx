...
export function EnvelopeIntro({ config, onOpen, onOpenCustomizer }: EnvelopeIntroProps) {
  ...
  const handleOpenEnvelope = () => {
    ...
    const introEndTime = config.introMusicEndTime && config.introMusicEndTime > introStartTime ? config.introMusicEndTime : (introStartTime + (config.introMusicDuration ?? 4));
    const introDuration = Math.max(1, introEndTime - introStartTime);
    const autoMute = config.introMusicAutoMute !== false;
    const introDelayMs = Math.max(0, (config.introMusicAutoPlayDelay ?? 0) * 1000);
    const isBgMusicEnabled = Boolean(config.bgMusicEnabled) && Boolean(config.bgMusicAutoPlay) && config.musicType !== 'none';

    if (isIntroEnabled && config.introMusicType) {
      const playIntroAction = () => {
        // Play intro music with manual start/end times
        sound.playIntroMusic(
          {
            type: config.introMusicType || 'happy_birthday_chime',
            startTime: introStartTime,
            endTime: introEndTime,
            duration: introDuration,
            audioUrl: config.introMusicAudioUrl,
            autoMute: autoMute,
          },
          () => {
            // Handover to background music if autoPlay is enabled
            if (isBgMusicEnabled) {
      ...
      if (introDelayMs > 0) {
        setTimeout(playIntroAction, introDelayMs);
      } else {
        playIntroAction();
      }

      // Unveil main page smoothly after intro completes (or transition at 2.5s for long intros)
      const transitionDelay = Math.min(introDelayMs + introDuration * 1000, 3500);
      setTimeout(() => {
        onOpen();
      }, transitionDelay);
    } else {
      // Intro music disabled: Start background music immediately if bgMusic is explicitly enabled & autoPlay is on
      if (isBgMusicEnabled) {
        sound.startBackgroundMusic({
          startTime: config.bgMusicStartTime ?? 0,
          endTime: config.bgMusicEndTime ?? 0,
